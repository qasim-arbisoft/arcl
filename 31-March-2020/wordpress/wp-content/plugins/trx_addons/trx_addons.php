...
// Current version
if ( ! defined( 'TRX_ADDONS_VERSION' ) ) define( 'TRX_ADDONS_VERSION', '1.6.34.3' );
...
if (!function_exists('trx_addons_get_privacy_text')) {
	function trx_addons_get_privacy_text() {
		...
		$get_theme_option = get_option( $theme_slug . '_options' );

		$page = get_option('wp_page_for_privacy_policy');
		$privacy_text = is_array( $get_theme_option ) && ! empty( $get_theme_option['privacy_text'] ) ? $get_theme_option['privacy_text'] : __( 'I agree that my submitted data is being collected and stored.', 'trx_addons' ) ;
		// Translators: Add url to the Privacy Policy page
		$privacy_link = $page ? sprintf( '%1$s <a href="%2$s" target="_blank">%3$s</a>',
			esc_html__( 'For further details on handling user data, see our', 'trx_addons' ),
			esc_url( get_permalink( $page ) ),
			esc_html__( 'Privacy Policy', 'trx_addons')
		) : '';

		return apply_filters( 'trx_addons_filter_privacy_text', wp_kses_post( "$privacy_text $privacy_link" ) );
	}
}
