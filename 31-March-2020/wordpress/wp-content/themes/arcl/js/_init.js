...
jQuery(document).ready(function() {
	...
	function lighthouseschool_init_media_elements(cont) {
		if (LIGHTHOUSESCHOOL_STORAGE['use_mediaelements'] && cont.find('audio:not(.inited),video:not(.inited)').length > 0) {
            if (window.mejs) {
	...
	function lighthouseschool_resize_video(cont) {
		...
		cont.find( 'video' ).each(
			function() {
				...
				var video     = jQuery( this ).addClass( 'lighthouseschool_resize' ).eq( 0 );
		...
		cont.find( '.video_frame iframe' ).each(
			function() {
				// If item now invisible
				if (jQuery( this ).hasClass( 'trx_addons_resize' ) || jQuery( this ).addClass( 'lighthouseschool_resize' ).parents( 'div:hidden,section:hidden,article:hidden' ).length > 0) {
					return;
				}
