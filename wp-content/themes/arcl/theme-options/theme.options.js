...
jQuery(document).ready(function() {
	...
	jQuery( window ).load( lighthouseschool_options_start_check_dependencies );
				...
				if (depend) {
					...
					for (var i in depend) {
						...
						if (name.charAt( 0 ) == '#' || name.charAt( 0 ) == '.') {
							fld = jQuery( name );
							if (fld.length > 0 && ! fld.hasClass( 'lighthouseschool_inited' )) {
								fld.addClass( 'lighthouseschool_inited' ).on(
									'change', function () {
										jQuery( '.lighthouseschool_options .lighthouseschool_options_section' ).each(
											function () {
												lighthouseschool_options_check_dependencies( jQuery( this ) );
					...
					if (((dep_cnt > 0 || dep_all == 0) && dep_cmp == 'or') || (dep_cnt == dep_all && dep_cmp == 'and')) {
						ctrl.parents( '.lighthouseschool_options_item' ).show().removeClass( 'lighthouseschool_options_no_use' );
					} else {
						ctrl.parents( '.lighthouseschool_options_item' ).hide().addClass( 'lighthouseschool_options_no_use' );
					}
				}
